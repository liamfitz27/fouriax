...
#SBATCH --job-name=rgb_e2e_cnn_grid_sweep
...
run_point() {
  ...
  printf -v artifacts_dir \
    "experiments/artifacts/rgb_e2e_cnn_grid_sweep_wl_%04d_%04d_distance_%04d" \
    "$wavelength_min_nm" \
    "$wavelength_max_nm" \
    "$distance_um"

  echo "distance_um=${distance_um} wavelength_nm=${wavelength_min_nm}-${wavelength_max_nm} artifacts_dir=${artifacts_dir}"
